...
public class AzureOpenAITextGenerator : ITextGenerator
{
        ...
        HttpClient? httpClient = null)
    {
        ...
        httpClient.Timeout = TimeSpan.FromMinutes(5); // Default timeout is not enough to get response
        OpenAIClientOptions options = new()
        {
            Transport = new HttpClientTransport(httpClient),
            Retry = {
                            // Configure the retry policy to use exponential backoff.
                            Mode = RetryMode.Exponential,
                            // Set the maximum number of retry attempts.
                            MaxRetries = Math.Max(0, config.MaxRetries),
                            // Set the delay between retries to start at 0.8 seconds.
                            Delay = TimeSpan.FromSeconds(0.8),
                            // Set the maximum delay between retries to 1 minute.
                            MaxDelay = TimeSpan.FromMinutes(1),
                            // Set the maximum duration to spend on retries.
